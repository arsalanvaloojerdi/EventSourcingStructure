using EventSource.Structure.Config.CommandApi;
...
namespace EventSource.Structure.CommandApi
{
	public class Startup
	{
		...
		private static void ConfigExceptionFilter(HttpConfiguration config)
		{
			...
			{
				new ExceptionInfoBuilder().WithType(typeof(CommandHandlerException)).WithMessage("An Error Occured").WithStatusCode(HttpStatusCode.BadRequest).Build()
			};
